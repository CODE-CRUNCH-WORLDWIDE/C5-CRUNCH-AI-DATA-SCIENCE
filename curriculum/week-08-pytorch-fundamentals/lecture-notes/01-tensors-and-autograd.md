...
```mermaid
flowchart LR
  X["x leaf, requires grad"] --> SQ["square op"]
  X --> MUL["multiply by 3 op"]
  SQ --> ADD1["add op"]
  MUL --> ADD1
  ADD1 --> ADD2["add 1 op"]
  ADD2 --> Y["y scalar loss"]
```
*Forward builds the DAG left to right for y equals x squared plus 3x plus 1; backward walks it right to left applying the chain rule at each op.*
...
You will rarely want this. It is documented because learners who do not know it spend an afternoon wondering why `.grad` is `None` on a non-leaf they thought should have it. The right read is: `.grad` lives on the leaves; everything else flows through.
...
- **Understanding.** A learner who only learns PyTorch will pass this week. A learner who only learns PyTorch will, in three months, hit a NaN loss and have no idea where to start because they have never written the backward pass. Week 7 is the inoculation.
