...
```mermaid
flowchart LR
  DS["Dataset getitem returns one example"] --> DL["DataLoader batches and shuffles"]
  DL --> WK["Workers prefetch in parallel, optional"]
  WK --> BT["Collated batch tensors"]
  BT --> DEV["Move batch to device and train"]
```
*One example becomes a device-ready training batch as it flows through the Dataset and DataLoader.*
...
For learners who want a harder benchmark, `torchvision.datasets.CIFAR10` provides 50,000 training and 10,000 test 32×32 color images across ten classes (airplane, automobile, bird, cat, deer, dog, frog, horse, ship, truck).
