...
```mermaid
flowchart TD
  Start["Start epoch"] --> Shuffle["Shuffle training indices"]
  Shuffle --> Batch["Take next mini-batch"]
  Batch --> Step["Run sgd_step: forward, backward, update"]
  Step --> More{"More batches this epoch"}
  More -->|"yes"| Batch
  More -->|"no"| Eval["Evaluate test accuracy"]
  Eval --> Next{"More epochs"}
  Next -->|"yes"| Start
  Next -->|"no"| Done["Return trained params"]
```
*The mini-batch SGD training loop: shuffle, step through batches, evaluate, repeat for n_epochs.*
...
```mermaid
flowchart TD
  Check["After each epoch"] --> S1{"Training loss plateaued"}
  S1 -->|"no"| Continue["Keep training"]
  S1 -->|"yes"| S2{"Test accuracy decreasing"}
  S2 -->|"yes"| Stop["Stop now - overfitting has begun"]
  S2 -->|"no"| StopOk["Stop soon - converged"]
```
*Three signals decide when to stop training: keep going, converged, or already overfitting.*
