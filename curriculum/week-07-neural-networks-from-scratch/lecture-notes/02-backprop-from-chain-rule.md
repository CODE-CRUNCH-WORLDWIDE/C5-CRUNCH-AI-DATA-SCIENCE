...
```mermaid
flowchart TD
  L["Loss L"] --> dZ2["dZ2 equals A2 minus Y over m"]
  dZ2 --> dW2["dW2 equals A1 transpose dot dZ2"]
  dZ2 --> db2["db2 equals sum of dZ2"]
  dZ2 --> dA1["dA1 equals dZ2 dot W2 transpose"]
  dA1 --> dZ1["dZ1 equals dA1 times ReLU mask"]
  dZ1 --> dW1["dW1 equals X transpose dot dZ1"]
  dZ1 --> db1["db1 equals sum of dZ1"]
```
*Gradients flow backward from the loss to each parameter, mirroring the forward pass in reverse.*
...
```mermaid
flowchart TD
  S["Something looks wrong"] --> Q1{"ValueError shapes not aligned"}
  Q1 -->|"yes"| B1["Bug 1: shape mismatch - check the transpose"]
  Q1 -->|"no"| Q2{"Gradient check error near batch size m"}
  Q2 -->|"yes"| B2["Bug 2: missing or doubled divide by m"]
  Q2 -->|"no"| Q3{"Loss increases during training"}
  Q3 -->|"yes"| B3["Bug 3: wrong sign in the update step"]
  Q3 -->|"no"| B4["Bug 4: unstable softmax producing nan"]
```
*A diagnostic decision tree for the four canonical backprop bugs, from symptom to cause.*
