...
```mermaid
flowchart LR
  X["Input X - m by 784"] --> L1["Linear W1 plus b1"]
  L1 --> Z1["Z1 pre-activation - m by h"]
  Z1 --> A1["ReLU to A1 - m by h"]
  A1 --> L2["Linear W2 plus b2"]
  L2 --> Z2["Z2 pre-activation - m by 10"]
  Z2 --> A2["Softmax to A2 - m by 10"]
```
*The two-layer MLP architecture: shapes flow from the 784-pixel input through the ReLU hidden layer to the 10-class softmax output.*
...
```mermaid
flowchart TD
  G["Compare predicted p_i to true label Y_i"] -->|"p_i greater than Y_i"| Down["Push logit z_i down"]
  G -->|"p_i less than Y_i"| Up["Push logit z_i up"]
```
*The softmax plus cross-entropy gradient always nudges each logit toward the true label.*
