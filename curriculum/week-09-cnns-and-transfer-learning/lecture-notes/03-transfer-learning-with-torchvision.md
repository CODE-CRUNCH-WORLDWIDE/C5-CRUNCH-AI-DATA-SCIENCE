...
```mermaid
flowchart TD
  P["Pretrained ResNet18 on ImageNet"] --> H["Replace head with new Linear layer"]
  H --> Q{"Freeze backbone"}
  Q -->|"Yes"| FE["Feature extraction train head only"]
  Q -->|"No"| FT["Fine tuning train whole network"]
  FE --> R1["85 to 87 percent in 5 epochs"]
  FT --> R2["90 to 93 percent in 10 epochs"]
```
*Two transfer-learning recipes branch from the same pretrained model and head swap.*
...
```mermaid
flowchart LR
  Stem["Stem frozen"] --> L1["Layer1 frozen"]
  L1 --> L2["Layer2 frozen"]
  L2 --> L3["Layer3 frozen"]
  L3 --> L4["Layer4 trainable"]
  L4 --> FC["FC head trainable"]
```
*Partial fine-tuning freezes the early general-purpose layers and trains only the last stage and head.*
