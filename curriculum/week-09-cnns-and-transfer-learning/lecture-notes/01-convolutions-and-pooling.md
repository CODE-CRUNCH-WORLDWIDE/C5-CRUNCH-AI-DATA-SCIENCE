...
```mermaid
mindmap
  root(("Convolution inductive biases"))
    ("Weight sharing")
      ("Same kernel reused at every position")
    ("Locality")
      ("Output depends on a small input window")
    ("Translation equivariance")
      ("Shift the input, output shifts the same way")
```
*The three architectural priors that let CNNs generalize from far fewer parameters than an MLP.*
...
```mermaid
flowchart LR
  A["Input pixel RF 1"] --> B["Conv1 kernel 3 RF 3"]
  B --> C["Pool1 kernel 2 RF 4"]
  C --> D["Conv2 kernel 3 RF 8"]
  D --> E["Pool2 kernel 2 RF 10"]
```
*Receptive field grows from a single pixel to a 10 by 10 window as it passes through four layers.*
