...
```mermaid
flowchart LR
  A["LeNet 1998 conv plus pooling"] --> B["AlexNet 2012 scale plus ReLU"]
  B --> C["VGG 2014 stack of 3x3 convs"]
  C --> D["ResNet 2015 residual connections"]
```
*Each architecture in the lineage answers a specific limitation of the one before it.*
...
```mermaid
flowchart TD
  X["Input x"] --> F1["Conv BN ReLU"]
  F1 --> F2["Conv BN"]
  X --> S["Identity or 1x1 shortcut"]
  F2 --> Add(("Add"))
  S --> Add
  Add --> R["Final ReLU"]
  R --> Y["Output y"]
```
*The residual block adds the input back to the transformed output before the final ReLU.*
