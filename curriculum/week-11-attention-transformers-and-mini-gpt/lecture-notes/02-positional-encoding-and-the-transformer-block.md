...
flowchart TD
  ...
  ADD2 --> Y["Block output"]
```
*Pre-norm transformer block: each sublayer normalizes, computes, then adds its result back into the residual stream.*
...
```mermaid
flowchart LR
  E["Token plus position embed"] --> A1["Attention writes"]
  A1 --> M1["MLP writes"]
  M1 --> A2["Attention writes"]
  A2 --> M2["MLP writes"]
  M2 --> U["Unembed reads"]
```
*The residual stream flows through the stack; each sublayer reads it, computes, and adds its own delta back.*
