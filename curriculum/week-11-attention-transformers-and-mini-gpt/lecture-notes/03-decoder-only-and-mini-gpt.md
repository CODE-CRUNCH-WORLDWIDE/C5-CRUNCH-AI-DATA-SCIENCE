...
```mermaid
flowchart TD
  TE["Token embedding"] --> ADD["Add"]
  PE["Position embedding"] --> ADD
  ADD --> BLOCKS["n layer transformer blocks"]
  BLOCKS --> LNF["Final LayerNorm"]
  LNF --> HEAD["Unembed projection"]
  HEAD --> LOSS["Cross entropy loss"]
```
*The six components of the decoder-only mini-GPT, from token ids to loss.*
...
```mermaid
flowchart TD
  P["Encode prompt"] --> F["Forward pass on current tokens"]
  F --> L["Take logits at last position"]
  L --> T["Divide by temperature and softmax"]
  T --> S["Sample next token"]
  S --> AP["Append to sequence"]
  AP --> CH{"Reached n chars"}
  CH -->|No| F
  CH -->|Yes| D["Done"]
```
*The autoregressive sampling loop: each new token requires a fresh forward pass over the growing sequence.*
