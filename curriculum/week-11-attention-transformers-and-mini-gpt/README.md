...
2. **We build attention by hand before we touch `nn.MultiheadAttention`.** Exercise 1 has you implement scaled dot-product attention from the equation `softmax(Q K^T / sqrt(d_k)) V` in pure PyTorch and verify against `torch.nn.functional.scaled_dot_product_attention`. Exercise 2 implements multi-head attention with explicit Q/K/V projections and head splitting. Only Exercise 3 (and the mini-project) use the fused `F.scaled_dot_product_attention` kernel. Learners who skip the hand-rolled attention cannot answer "where does the `sqrt(d_k)` come from?" in interview.
