...
flowchart LR
  ...
  B --> E["Tree 1 - random feature subset"]
  C --> F["Tree 2 - random feature subset"]
  D --> G["Tree B - random feature subset"]
  E --> H["Average predictions"]
  F --> H
  G --> H
```
*Each tree trains on its own bootstrap sample and a random subset of features; the forest averages their predictions.*
...
```mermaid
flowchart LR
  A["Row i"] --> B["Find trees that did not train on row i"]
  B --> C["Average those trees predictions"]
  C --> D["Compare to true y for row i"]
  D --> E["OOB score - free validation"]
```
*Out-of-bag rows give each row a prediction from only the trees that never trained on it, yielding a validation score with no held-out set.*
