...
```mermaid
flowchart TD
  A["Node with data"] --> B{"Stop condition met"}
  B -->|Yes| C["Make leaf - predict mean"]
  B -->|No| D["Find best split"]
  D --> E{"Gain greater than zero"}
  E -->|No| C
  E -->|Yes| F["Split into left and right"]
  F --> G["Recurse on left child"]
  F --> H["Recurse on right child"]
```
*The greedy recursive-split algorithm: every node either becomes a leaf or splits and recurses on both children.*
