...
```mermaid
flowchart TD
  A["Start - f0 is mean of y"] --> B["Compute residuals"]
  B --> C["Fit tree h to residuals"]
  C --> D["Shrink by learning rate nu"]
  D --> E["Add to ensemble"]
  E --> F{"More rounds remaining"}
  F -->|Yes| B
  F -->|No| G["Final prediction F of x"]
```
*Each round fits a small tree to the current residuals and adds a shrunk copy to the running ensemble.*
...
```mermaid
flowchart TD
  A["Train one more boosting round"] --> B["Evaluate on validation set"]
  B --> C{"Validation score improved"}
  C -->|Yes| D["Reset patience counter"]
  D --> A
  C -->|No| E["Increment patience counter"]
  E --> F{"Patience exceeds stopping rounds"}
  F -->|No| A
  F -->|Yes| G["Stop - use best round so far"]
```
*Early stopping keeps training until the validation score stalls for stopping_rounds consecutive rounds, then rolls back to the best round.*
