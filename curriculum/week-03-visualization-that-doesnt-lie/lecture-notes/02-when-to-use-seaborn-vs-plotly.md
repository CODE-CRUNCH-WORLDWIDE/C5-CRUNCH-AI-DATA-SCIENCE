...
```mermaid
flowchart TD
  Q["Pick the question type"] --> T1["Distribution - histplot or boxplot"]
  Q --> T2["Relationship - scatter or regplot"]
  Q --> T3["Composition - stacked bar not pie"]
  Q --> T4["Change over time - line chart"]
  Q --> T5["Ranking - horizontal sorted bar"]
  T1 --> I["Will the reader interact"]
  T2 --> I
  T3 --> I
  T4 --> I
  T5 --> I
  I -->|Yes| P["plotly express"]
  I -->|No| M["matplotlib plus seaborn"]
```
*Chart type follows the question first, then interactivity decides the library.*
