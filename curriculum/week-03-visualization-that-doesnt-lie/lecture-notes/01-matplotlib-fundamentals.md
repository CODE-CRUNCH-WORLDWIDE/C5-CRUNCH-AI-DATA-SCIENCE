...
flowchart TD
  F["Figure"] --> A["Axes"]
  F --> ST["Suptitle"]
  A --> XA["XAxis - ticks and label"]
  A --> YA["YAxis - ticks and label"]
  A --> SP["Spines - four edges"]
  A --> AR["Artists - Line2D Rectangle etc"]
  A --> LG["Legend"]
  A --> TI["Title"]
```
*The Figure owns one or more Axes, and each Axes owns its own axes, spines, artists, legend, and title.*
