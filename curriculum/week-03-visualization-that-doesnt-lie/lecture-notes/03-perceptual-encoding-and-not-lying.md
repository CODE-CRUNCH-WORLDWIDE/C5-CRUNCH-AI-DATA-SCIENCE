...
```mermaid
flowchart TD
  A["Position on a common scale - most accurate"] --> B["Position on non-aligned scales"]
  B --> C["Length - bar chart"]
  C --> D["Angle or slope"]
  D --> E["Area"]
  E --> F["Volume - avoid"]
  F --> G["Color saturation"]
  G --> H["Color hue - categories only"]
```
*The Cleveland and McGill perceptual ladder, from most to least accurately decoded by the eye.*
...
```mermaid
flowchart TD
  L["Common chart lies"] --> L1["Truncated bar baseline"]
  L --> L2["Dual axis correlation theater"]
  L --> L3["Rainbow colormap on ordinal data"]
  L --> L4["3D anything"]
  L1 --> F1["Fix - start bars at zero"]
  L2 --> F2["Fix - normalize or small multiples"]
  L3 --> F3["Fix - use viridis or RdBu"]
  L4 --> F4["Fix - go 2D"]
```
*Each of the four common lies has a one-line honest fix.*
