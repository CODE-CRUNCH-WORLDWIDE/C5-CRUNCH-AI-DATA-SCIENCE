...
```mermaid
flowchart LR
  A["s plus equals x"] --> B["call float dot iadd"]
  B --> C["call float dot add"]
  C --> D["allocate new PyFloatObject"]
  D --> E["rebind name s"]
```
*Each Python-level addition on a list element pays for a method call, a fresh allocation, and a rebind.*
...
```mermaid
flowchart TD
  A["Array operation"] --> B{"Basic slicing reshape or transpose"}
  B -->|Yes| C["View - shares memory"]
  B -->|No| D{"Boolean mask or fancy indexing"}
  D -->|Yes| E["Copy - new buffer"]
  D -->|No| F{"astype or arithmetic"}
  F -->|Yes| E
```
*Most shape-only operations hand back a view; anything that gathers or transforms values hands back a copy.*
