...
```mermaid
flowchart TD
  A["Compare shapes right to left, axis by axis"] --> B{"Dimensions equal"}
  B -->|Yes| C["Compatible"]
  B -->|No| D{"One dimension is 1"}
  D -->|Yes| C
  D -->|No| E{"One dimension is missing"}
  E -->|Yes| C
  E -->|No| F["Broadcasting error"]
```
*NumPy's three compatibility checks, applied from the last axis backward until one side runs out of axes.*
...
```mermaid
flowchart LR
  A["Array shape 3 by 4"] --> B{"Which axis"}
  B -->|"axis=0"| C["Collapses rows - result shape 4"]
  B -->|"axis=1"| D["Collapses columns - result shape 3"]
```
*The axis you name is the axis that disappears from the result.*
