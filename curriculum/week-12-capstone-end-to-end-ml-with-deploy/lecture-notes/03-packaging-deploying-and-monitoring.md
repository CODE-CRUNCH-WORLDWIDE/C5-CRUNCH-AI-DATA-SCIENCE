...
```mermaid
flowchart LR
  A["Incoming request"] --> B["validate_row checks keys and types"]
  B --> C["predict_one calls the model"]
  C --> D["load_model cached after first call"]
  D --> E["JSON result returned"]
```
*A request's path through the inference layer, from raw input to JSON response.*
...
```mermaid
flowchart TD
  A["Trained model dot joblib"] --> B["Need a UI demo"]
  A --> C["Need a dashboard"]
  A --> D["Need an HTTP API"]
  B --> E["Hugging Face Spaces plus Gradio"]
  C --> F["Streamlit Community Cloud"]
  D --> G["Fly dot io plus FastAPI"]
```
*Which deploy target fits which capstone shape.*
