...
```mermaid
flowchart TD
  A["Blank page state"] --> B["What kind of problem is this"]
  B --> C["What is the prediction target"]
  C --> D["What is the metric in units"]
  D --> E["What is the baseline as a number"]
  E --> F["Recorded in PROBLEM dot md"]
```
*The four questions that turn a blank-page capstone into a tractable one-week project.*
...
```mermaid
flowchart LR
  A["Full dataset"] --> B["Split off test 15 percent"]
  B --> C["Remaining pool 85 percent"]
  C --> D["Train 70 percent"]
  C --> E["Validation 15 percent"]
  D --> F["Fit the model"]
  E --> G["Tune and early stop"]
  B --> H["Test set touched once at the end"]
```
*Test is carved off first and never touched again until the final headline metric.*
