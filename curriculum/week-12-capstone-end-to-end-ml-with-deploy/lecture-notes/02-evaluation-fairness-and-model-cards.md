...
```mermaid
flowchart TD
  A["Per-slice metric computed"] --> B["Best slice versus worst slice gap"]
  B --> C["Gap at or under 5 points - no action"]
  B --> D["Gap over 5 points - mitigation needed"]
  D --> E["Rung 1 document the gap"]
  E --> F["Rung 2 rebalance and retrain"]
  F --> G["Rung 3 constrained optimization"]
```
*From a measured slice gap to the mitigation rung it triggers.*
...
```mermaid
flowchart LR
  A["Dataset - DVC or pinned Hub revision"] --> D["Reproducible training run"]
  B["Model - SHA256 hash"] --> D
  C["Training code - git commit SHA"] --> D
  D --> E["Model card records all three"]
```
*Three independently versioned pieces combine into one reproducible run.*
