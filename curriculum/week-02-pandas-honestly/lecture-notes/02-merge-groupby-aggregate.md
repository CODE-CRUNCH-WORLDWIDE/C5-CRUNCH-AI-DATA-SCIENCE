...
```mermaid
flowchart TD
    A["Choose a merge how value"] --> B{"Keep only matching keys"}
    B -->|"Yes"| C["how equals inner"]
    B -->|"No"| D{"Keep every row from the left table"}
    D -->|"Yes, right can have gaps"| E["how equals left"]
    D -->|"No"| F{"Keep every row from the right table"}
    F -->|"Yes, left can have gaps"| G["how equals right"]
    F -->|"No, keep everything from both"| H["how equals outer"]
```
*Picking the right how value based on which rows you must keep.*
...
```mermaid
flowchart LR
    A["Rows in one DataFrame"] --> B["Split by group key"]
    B --> C["Group one"]
    B --> D["Group two"]
    B --> E["Group three"]
    C --> F["Apply a function to each group"]
    D --> F
    E --> F
    F --> G["Combine results back together"]
```
*Split apply combine: one DataFrame becomes groups, each group is processed, results come back combined.*
