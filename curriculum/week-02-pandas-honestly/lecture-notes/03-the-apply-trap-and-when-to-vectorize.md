...
```mermaid
flowchart TD
    A["About to write apply"] --> B{"Arithmetic on a Series"}
    B -->|"Yes"| Z1["Use an operator"]
    B -->|"No"| C{"String or datetime operation"}
    C -->|"Yes"| Z2["Use the str or dt accessor"]
    C -->|"No"| D{"Conditional if then else"}
    D -->|"Yes"| Z3["Use np where, np select, or pd cut"]
    D -->|"No"| E{"Lookup or per-group aggregation"}
    E -->|"Yes"| Z4["Use map, merge, or groupby agg"]
    E -->|"No"| F["apply may be honest - benchmark it"]
```
*The pre-flight checklist as a decision path: most roads lead away from apply.*
