...
```mermaid
flowchart TD
    A["Selecting or setting data"] --> B{"Index by label or position"}
    B -->|"Label"| C["Use loc"]
    B -->|"Position"| D["Use iloc, zero based, half open"]
    C --> E{"Setting a value, or masking plus a column"}
    E -->|"Yes"| F["Always use loc for that write"]
    E -->|"No - simple single column read"| G["Plain df bracket col is fine"]
```
*Deciding between loc and iloc, and when a plain df bracket read is still safe.*
...
```mermaid
flowchart LR
    subgraph Chained indexing - risky
        A["df bracket salary"] --> B["Series returned - view or copy unknown"]
        B --> C["Mask then assign"]
        C --> D["SettingWithCopyWarning"]
    end
    subgraph "Single loc call - safe"
        E["One loc call with mask and column"] --> F["Rows and column both known"]
        F --> G["DataFrame updated cleanly"]
    end
```
*Two steps on an ambiguous object versus one unambiguous loc call.*
