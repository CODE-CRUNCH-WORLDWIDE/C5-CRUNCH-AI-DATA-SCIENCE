...
**B** — The validation set exists so that picking hyperparameters and comparing models does not "spend" the test set. Every comparison leaks a little optimism into the validation score; the test set is held back precisely to catch that optimism at the end. Option A is false (`train_test_split` returns one split per call). Option C is wrong (two-way splits are fine for very large datasets). Option D is unrelated to the existence of a validation set.
