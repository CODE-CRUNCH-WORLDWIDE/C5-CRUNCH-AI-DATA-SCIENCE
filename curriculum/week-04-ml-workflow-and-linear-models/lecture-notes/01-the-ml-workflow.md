...
```mermaid
flowchart TD
  A["Frame the problem"] --> B["Build the dataset"]
  B --> C["Split train validation test"]
  C --> D["Baseline"]
  D --> E["Fit simple model"]
  ...
  H --> I["Write it up"]
```
*The nine-step ML workflow — steps 5 through 7 loop until validation score stops improving.*
...
```mermaid
flowchart TD
  A["Read the learning curve"] --> B{"Train and validation gap"}
  B -->|"Both high and close"| C["High bias - too simple"]
  B -->|"Train low validation high"| D["High variance - too flexible"]
  B -->|"Both still dropping"| E["Keep going"]
  C --> F["Add features or raise complexity"]
  D --> G["Add regularization or more data"]
```
*Diagnosing bias versus variance from the shape of the learning curve.*
