...
```mermaid
flowchart TD
  A["Linear regression inference assumptions"] --> B["Linearity"]
  A --> C["Independence of errors"]
  A --> D["Normality of errors"]
  A --> E["Homoscedasticity"]
  A --> F["No perfect multicollinearity"]
```
*The five classical assumptions behind interpreting coefficients and p-values, not the model fit itself.*
...
```mermaid
flowchart LR
  A["Fold training rows"] --> B["Fit scaler on fold"]
  B --> C["Transform fold train and fold validation"]
  C --> D["Fit ridge on transformed train"]
  D --> E["Score on transformed validation"]
  E --> F["Repeat for next fold"]
  F --> B
```
*A leak-free pipeline re-fits the scaler on every fold instead of once on the whole dataset.*
