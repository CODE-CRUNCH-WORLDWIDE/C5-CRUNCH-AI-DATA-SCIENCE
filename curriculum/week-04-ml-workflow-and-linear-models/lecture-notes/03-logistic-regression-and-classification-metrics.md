...
```mermaid
flowchart LR
  A["Linear score x times beta"] --> B["Sigmoid gives probability"]
  B --> C["Compare to threshold"]
  C --> D["Predicted class 0 or 1"]
  D --> E["Confusion matrix counts"]
  E --> F["Precision recall F1 ROC-AUC"]
```
*From a raw score to a shipped metric: sigmoid, threshold, confusion matrix, then the numbers you report.*
...
```mermaid
flowchart TD
  A["Choosing a headline metric"] --> B{"Data imbalanced"}
  B -->|"Yes"| C["Use PR-AUC not ROC-AUC"]
  B -->|"No"| D["ROC-AUC is fine"]
  A --> E{"Costs asymmetric"}
  E -->|"False positives expensive"| F["Report precision"]
  E -->|"False negatives expensive"| G["Report recall"]
  E -->|"Need one number"| H["Report F1"]
```
*Picking the right classification metric from class balance and the cost of each error type.*
