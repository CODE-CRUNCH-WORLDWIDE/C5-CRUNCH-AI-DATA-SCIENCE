...
2. In the low-dimensional embedding `Y`, define an analogous conditional probability `Q(j | i)` using a *Learner t* distribution with one degree of freedom (the t-distribution has heavier tails than the Gaussian, which avoids "crowding" in low dimensions).
...
```mermaid
flowchart LR
  A["High dimensional data"] --> B["Neighbor probabilities P using Gaussian kernel"]
  B --> C["Low dimensional points"]
  C --> D["Neighbor probabilities Q using t distribution"]
  D --> E["Minimize KL divergence P versus Q"]
  E --> C
```
*t-SNE moves the low-dimensional points to make Q match the high-dimensional neighbor probabilities P.*
...
```mermaid
flowchart TD
  A{"Is the structure linear"} -- Yes --> B["PCA"]
  A -- No --> C{"Need transform for new data or large n"}
  C -- Yes --> D["UMAP"]
  C -- No --> E["t-SNE for visualization only"]
```
*Picking among PCA, t-SNE, and UMAP based on linearity and the need to transform new data.*
