...
```mermaid
flowchart TD
  A["What does the user actually want"] --> B["Segments on continuous data"]
  A --> C["Prediction of a known label"]
  A --> D["Unusual or rare rows"]
  A --> E["Nested sub-groups"]
  A --> F["A visual map of rows"]
  B --> B1["Quantile based segmentation"]
  C --> C1["Classification model"]
  D --> D1["Anomaly detection"]
  E --> E1["Hierarchical clustering"]
  F --> F1["Dimensionality reduction plot"]
```
*Five requests that sound like clustering but map to a different tool.*
...
```mermaid
flowchart TD
  A["Plot the elbow"] --> B["Compute silhouette and diagram"]
  B --> C["Reshuffle columns and refit"]
  C --> D["Refit with different random states"]
  D --> E["Plot PCA and UMAP embeddings"]
  E --> F{"Four or five signs say real"}
  F -- Yes --> G["Clusters are real"]
  F -- No --> H["Clusters are weak or artifact"]
```
*The five-step protocol for deciding whether a clustering result is real.*
