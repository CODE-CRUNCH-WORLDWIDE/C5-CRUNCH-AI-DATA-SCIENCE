...
```mermaid
flowchart TD
  A["Pick k initial centroids"] --> B["Assign each row to nearest centroid"]
  B --> C["Update each centroid to cluster mean"]
  C --> D{"Did labels change"}
  D -- Yes --> B
  D -- No --> E["Stop converged"]
```
*Lloyd's algorithm alternates between assigning rows and updating centroids until labels stop changing.*
...
```mermaid
flowchart TD
  A{"Round equal density scaled clusters"} -- Yes --> B["K-means"]
  A -- No --> C{"Need dendrogram or nested structure"}
  C -- Yes --> D["Hierarchical Ward linkage"]
  C -- No --> E{"Arbitrary shape clusters with noise"}
  E -- Yes --> F["DBSCAN"]
  E -- No --> G["Gaussian Mixture Model"]
```
*A rough decision path from data shape to the right clustering algorithm.*
