...
flowchart LR
  ...
  X3["x_3"] --> H3["h_3"]
  H2 --> H3
  H3 --> HT["h_T after T steps"]
```
*Unrolling the RNN recurrence across timesteps: each hidden state depends on the input at that step and the previous hidden state.*
...
```mermaid
flowchart TD
  A["Spectral norm of W_hh"] --> B["Less than 1"]
  A --> C["Equal to 1"]
  A --> D["Greater than 1"]
  B --> E["Vanishing gradient shrinks geometrically"]
  C --> F["Balanced norm stays bounded"]
  D --> G["Exploding gradient grows geometrically"]
```
*How the spectral norm of the recurrent weight matrix determines whether gradients vanish, stay balanced, or explode.*
