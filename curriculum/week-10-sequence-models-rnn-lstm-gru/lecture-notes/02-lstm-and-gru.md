...
```mermaid
flowchart TD
  X["x_t and h_t-1"] --> F["Forget gate f_t"]
  X --> I["Input gate i_t"]
  X --> G["Candidate g_t"]
  X --> O["Output gate o_t"]
  Cprev["Cell state c_t-1"] --> F
  F --> C["New cell state c_t"]
  I --> C
  G --> C
  C --> O
  O --> H["Hidden state h_t"]
```
*Data flow through one LSTM cell: gates control what is forgotten, written, and exposed as the new hidden state.*
...
```mermaid
flowchart TD
  X["x_t and h_t-1"] --> R["Reset gate r_t"]
  X --> Z["Update gate z_t"]
  R --> Cand["Candidate g_t using reset h_t-1"]
  X --> Cand
  Hprev["Previous hidden state h_t-1"] --> Blend["Convex combination"]
  Cand --> Blend
  Z --> Blend
  Blend --> H["New hidden state h_t"]
```
*GRU merges the cell state into the hidden state and uses two gates to blend old and new information.*
