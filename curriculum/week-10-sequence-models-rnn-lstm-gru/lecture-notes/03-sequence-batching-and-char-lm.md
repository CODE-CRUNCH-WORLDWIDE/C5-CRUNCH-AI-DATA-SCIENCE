...
```mermaid
flowchart LR
  Pad["Padded batch and lengths"] --> Pack["pack_padded_sequence"]
  Pack --> LSTM["LSTM layer"]
  LSTM --> Unpack["pad_packed_sequence"]
  Unpack --> Out["Padded output tensor"]
```
*The packing pipeline lets the LSTM skip padded positions while still returning a padded output tensor.*
...
```mermaid
flowchart LR
  Chunk1["Chunk 1 forward and backward"] --> Detach1["Detach hidden state"]
  Detach1 --> Chunk2["Chunk 2 forward and backward"]
  Chunk2 --> Detach2["Detach hidden state"]
  Detach2 --> Chunk3["Chunk 3 forward and backward"]
```
*Truncated BPTT: hidden-state values pass forward between chunks but gradients are cut off at each detach point.*
